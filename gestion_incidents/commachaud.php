...
Set objOL = CreateObject("Outlook.Application")
	...
	content = content & "<?= $incident->getChronogramme(); ?>"
	content = content & "</td></tr><tr>"
...
With objMail
		.SentOnBehalfOfName = ""
		.Subject = "{<?= $STATUTCOMMACHAUD[$incident->getStatut()]; ?>} - [<?= $incident->getIncident();?>] - <?php if($incident->getPriorite()) echo '['.$PRIORITE[$incident->getPriorite()].']';?> - [RETAIL][<?= $application->getEnseigne();?>][<?= $application->getName();?>] - <?= $incident->getTitre();?>"
		.To = "<?= DESTINATAIRE; ?>"
		.Cc = "<?= DESTINATAIRECC; ?>"
		.BCC = "<?= DESTINATAIREBCC; ?>"
		.HTMLBody = content
